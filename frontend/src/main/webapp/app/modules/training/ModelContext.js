...
export const ModelContextProvider = ({ children }) => {
  ...
  const initialBoilerplate = `#!pip install boto3 matplotlib torch torchvision scikit-learn neo4j onnx 
import shutil
import os
import boto3
import neo4j
...
try:
  ...
  `;
  // Load boilerplate from localStorage or use initialBoilerplate
  const [boilerplate, setBoilerplate] = useState(() => {
    const savedBoilerplate = localStorage.getItem('boilerplate');
    return savedBoilerplate !== null && savedBoilerplate !== '' ? savedBoilerplate : initialBoilerplate;
  });

  // Save initialBoilerplate to localStorage if not already saved
  useEffect(() => {
    if (localStorage.getItem('boilerplate') === null) {
      localStorage.setItem('boilerplate', initialBoilerplate);
    }
  }, [initialBoilerplate]);

  // Update localStorage whenever boilerplate changes
  useEffect(() => {
    localStorage.setItem('boilerplate', boilerplate);
  }, [boilerplate]);

  const valueStored = useRef([]);
  const maxInChannelValStored = useRef([]);
  const linearOutFeatureStored = useRef([]); 
  const scriptStored = useRef([]); 
  const pyTorchScriptStored = useRef([]); 
  const [flattenStepNum, setFlattenStep] = useState("");
  const [numOfCategorySelected, setNumOfCategorySelected] = useState("");
  ...
  useEffect(() => {
    // Reset the ref to an empty array on location change
    valueStored.current = [];
    scriptStored.current = [];
    pyTorchScriptStored.current = []; 
    linearOutFeatureStored.current = [];

    setFlattenStep("");
    setNumOfCategorySelected("");
  ...
  return (
    <ModelContext.Provider 
      value={ { boilerplate, setBoilerplate, initialBoilerplate, 
                valueStored, scriptStored, pyTorchScriptStored, linearOutFeatureStored, 
                flattenStepNum, setFlattenStep, 
                numOfCategorySelected, setNumOfCategorySelected, 
                selectedCategories, setSelectedCategories, 
                selectedFolder, setSelectedFolder, 
                showAlert, setShowAlert, 
                alertMessage, setAlertMessage,
                existingModel, setExistingModel,
                selectedExistingModel, setSelectedExistingModel, model, setModel, preSetmodel, setPreSetModel} }>
      {children}
    </ModelContext.Provider>
  );
  };
