export const config = {
    port: 8080,
    origin: "https://uat-demo.link",
    errorRedirectUrl: "https://uat-demo.link/oauth/error",
    backendUrl: "https://api.uat-demo.link",
    domain: "uat-demo.link",
    secure: true,
    initialModelOptions: ["Conv2d"],
    neo4jUri: "neo4j://xxxxxxxxxxxxx.ap-southeast-1.compute.amazonaws.com:7687",
    neo4jUser: "neo4j",
    neo4jPassword: "panther-xxxxxxxxxxxxx",
    dbUri: "mongodb://xxxxxxxxxxxxx.ap-southeast-1.compute.amazonaws.com:27017/",
    awsAccessKeyId:"AKIAXIxxxxxxxxxxxxx", 
    awsSecretAccessKey:"wH74YaSxxxxxxxxxxxxx", 
    regionName:"ap-southeast-1",
    saltWorkFactor: 10, // Number of rounds to salt the password
    accessTokenTtl: "15m",
    refreshTokenTtl: "15m",
    publicKey: `-----BEGIN PUBLIC KEY-----
xxxxxxxxxxxxx

CQIDAQAB
-----END PUBLIC KEY-----`,
      privateKey: `-----BEGIN PRIVATE KEY-----
xxxxxxxxxxxxx

-----END PRIVATE KEY-----`,

    googleClientId:
    "644275109199-xxxxxxxxxxxxx.apps.googleusercontent.com",
    googleClientSecret: "GOCSPX-xxxxxxxxxxxxx",
    googleOauthRedirectUrl: "https://api.uat-demo.link/api/sessions/oauth/google",
};





  